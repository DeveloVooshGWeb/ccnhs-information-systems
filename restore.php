...
<?php
$searched = mysqli_real_escape_string($con, $_GET['searched']);
...
<?php
if (isset($_GET['id'])) {
$torestore = mysqli_query($con, "SELECT * FROM archived WHERE ID='". $_GET['id'] ."'");
$numoffound = mysqli_num_rows($torestore);
if ($numoffound > 0) {
$acs = mysqli_fetch_assoc($torestore);
mysqli_query($con, "INSERT INTO student_records VALUES (". $acs['ID'] .", '". $acs['Firstname'] ."', '". $acs['Middlename'] ."', '". $acs['Initial'] ."', '". $acs['Lastname'] ."', ". $acs['Age'] .", '". $acs['Gender'] ."', ". $acs['Grade'] .", '". $acs['Section'] ."', ". $acs['SY'] .", ". $acs['LRN'] .")");
mysqli_query($con, "DELETE FROM archived WHERE ID='". $_GET['id'] ."'");
echo "<newt>". $numoffound ." Record/Records RESTORED!!!</newt>";
} else {
